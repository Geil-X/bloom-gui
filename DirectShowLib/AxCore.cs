...
public interface IEnumPins
{
    ...
    /// </returns>
    /// <remarks>
    ///     This method retrieves pointers to the specified number of pins, starting at the current position in the
    ///     enumeration, and places them in the
    ///     specified array. If the method succeeds, the IPin pointers all have outstanding reference counts.Be sure to release
    ///     them when you are done.
    ///     If the number of pins changes, the enumerator is no longer consistent with the filter, and the method returns
    ///     VFW_E_ENUM_OUT_OF_SYNC.
    ///     Discard any data obtained from previous calls to the enumerator, because it might be invalid.Update the enumerator
    ///     by calling the
    ...
    /// </summary>
    /// <param name="ppEnum">
    ///     Receives a pointer to the <see cref="IEnumPins" /> interface of the new enumerator. The caller
    ...
    ///     Discard any data obtained from previous calls to the enumerator, because it might be invalid. Update the enumerator
    ///     by calling the
    ///     <see cref="IEnumPins.Reset" /> method. You can then call the Clone method safely.
    /// </remarks>
    [PreserveSig]
...
public interface IEnumMediaTypes
{
    ...
    int Next(
        [In] int cMediaTypes,
        [In] [Out] [MarshalAs(UnmanagedType.CustomMarshaler, MarshalTypeRef = typeof(EMTMarshaler), SizeParamIndex = 0)]
