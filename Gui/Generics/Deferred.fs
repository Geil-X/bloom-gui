...
module Deferred =
    ...
    /// Transforms the underlying value of the input deferred value when it exists
    ...
    /// Verifies that a `Deferred<'T>` value is resolved and the resolved data
    ...
    /// Like `map` but instead of transforming just the value into another type
    /// in the `Resolved` case, it will transform the value into potentially a
    /// different case of the the `Deferred<'T>` type.
