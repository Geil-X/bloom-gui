...
let fromBoundingBox (b: BoundingBox2D<Meters, 'Coordinates>) attr : IView<Rectangle> =
    Rectangle.create (
        [ Rectangle.width (Length.inCssPixels (BoundingBox2D.width b))
          Rectangle.height (Length.inCssPixels (BoundingBox2D.height b))
